...
-- 1. Cursor hiển thị thông tin khách hàng và tổng sách mượn
DECLARE @MADG VARCHAR(10), @TENDG NVARCHAR(50), @TONGSOSH INT;

DECLARE DSSOLUONGSACHMUON CURSOR FOR
SELECT MAKH, TENKH, COUNT(MAHG) AS TONGSOSH
FROM HOADON HD
JOIN CHITIETHD CTHD ON HD.MAHD = CTHD.MAHD
JOIN KHACHHG KH ON HD.MAKH = KH.MAKH
GROUP BY MAKH, TENKH;

OPEN DSSOLUONGSACHMUON;
FETCH NEXT FROM DSSOLUONGSACHMUON INTO @MADG, @TENDG, @TONGSOSH;

WHILE @@FETCH_STATUS = 0
BEGIN
    PRINT @MADG + ' - ' + @TENDG + ' - ' + CAST(@TONGSOSH AS VARCHAR);
    FETCH NEXT FROM DSSOLUONGSACHMUON INTO @MADG, @TENDG, @TONGSOSH;
END

CLOSE DSSOLUONGSACHMUON;
DEALLOCATE DSSOLUONGSACHMUON;


-- 2. Cursor hiển thị tổng nhập, xuất, tồn kho theo hàng
DECLARE @MAHG VARCHAR(10), @TENHG NVARCHAR(100), @SLNHAP INT, @SLXUAT INT, @SLCON INT;

DECLARE DSSOLUONGHANG CURSOR FOR
SELECT H.MAHG, H.TENHG,
       ISNULL(SUM(CPN.SOLUONG),0) AS SLNHAP,
       ISNULL(SUM(CTHD.SOLUONG),0) AS SLXUAT,
       H.SOLUONGTON AS SLCON
FROM HANG H
LEFT JOIN CHITIETPN CPN ON H.MAHG = CPN.MAHG
LEFT JOIN CHITIETHD CTHD ON H.MAHG = CTHD.MAHG
GROUP BY H.MAHG, H.TENHG, H.SOLUONGTON;

OPEN DSSOLUONGHANG;
FETCH NEXT FROM DSSOLUONGHANG INTO @MAHG, @TENHG, @SLNHAP, @SLXUAT, @SLCON;

WHILE @@FETCH_STATUS = 0
BEGIN
    PRINT @MAHG + ' - ' + @TENHG + ' - Nhập: ' + CAST(@SLNHAP AS VARCHAR)
          + ', Xuất: ' + CAST(@SLXUAT AS VARCHAR)
          + ', Tồn: ' + CAST(@SLCON AS VARCHAR);
    FETCH NEXT FROM DSSOLUONGHANG INTO @MAHG, @TENHG, @SLNHAP, @SLXUAT, @SLCON;
END

CLOSE DSSOLUONGHANG;
DEALLOCATE DSSOLUONGHANG;


-- 3. Cursor hiển thị chi tiết hóa đơn theo mã hóa đơn
DECLARE @MAHD VARCHAR(10), @MAHG VARCHAR(10), @TENHG NVARCHAR(100), @SOLUONG INT, @GIABAN MONEY, @THANHTIEN MONEY;

DECLARE DSSOLIEUHOADON CURSOR FOR
SELECT HD.MAHD, H.MAHG, H.TENHG, CTHD.SOLUONG, CTHD.GIABAN, CTHD.THANHTIEN
FROM HOADON HD
JOIN CHITIETHD CTHD ON HD.MAHD = CTHD.MAHD
JOIN HANG H ON CTHD.MAHG = H.MAHG;

OPEN DSSOLIEUHOADON;
FETCH NEXT FROM DSSOLIEUHOADON INTO @MAHD, @MAHG, @TENHG, @SOLUONG, @GIABAN, @THANHTIEN;

WHILE @@FETCH_STATUS = 0
BEGIN
    PRINT 'HD: ' + @MAHD + ' | ' + @MAHG + ' - ' + @TENHG 
          + ' | SL: ' + CAST(@SOLUONG AS VARCHAR)
          + ' | Giá: ' + CAST(@GIABAN AS VARCHAR)
          + ' | TT: ' + CAST(@THANHTIEN AS VARCHAR);
    FETCH NEXT FROM DSSOLIEUHOADON INTO @MAHD, @MAHG, @TENHG, @SOLUONG, @GIABAN, @THANHTIEN;
END

CLOSE DSSOLIEUHOADON;
DEALLOCATE DSSOLIEUHOADON;


-- 1. Cursor hiển thị tổng số sách đã mua của mỗi khách hàng
DECLARE @MAKH VARCHAR(10), @TENKH NVARCHAR(50), @TONGSACH INT;

DECLARE DSSOLUONGSACH CURSOR FOR
SELECT MAKH, TENKH,
       (SELECT COUNT(MAHG) 
        FROM CHITIETHD 
        WHERE MAHD IN (SELECT MAHD FROM HOADON WHERE MAKH = KH.MAKH)) AS TONGSACH
FROM KHACHHG KH;

OPEN DSSOLUONGSACH;
FETCH NEXT FROM DSSOLUONGSACH INTO @MAKH, @TENKH, @TONGSACH;

WHILE @@FETCH_STATUS = 0
BEGIN
    PRINT @MAKH + ' - ' + @TENKH + ' - ' + CAST(@TONGSACH AS VARCHAR);
    FETCH NEXT FROM DSSOLUONGSACH INTO @MAKH, @TENKH, @TONGSACH;
END

CLOSE DSSOLUONGSACH;
DEALLOCATE DSSOLUONGSACH;


-- 2. Cursor hiển thị tổng nhập, tổng xuất, tồn kho theo hàng
DECLARE @MAHG VARCHAR(10), @TENHG NVARCHAR(50), @SLNHAP INT, @SLXUAT INT, @SLCON INT;

DECLARE DSSOLUONGHANG CURSOR FOR
SELECT MAHG, TENHG,
       (SELECT ISNULL(SUM(SOLUONG),0) FROM CHITIETPN WHERE MAHG = HANG.MAHG) AS SLNHAP,
       (SELECT ISNULL(SUM(SOLUONG),0) FROM CHITIETHD WHERE MAHG = HANG.MAHG) AS SLXUAT,
       SOLUONGTON AS SLCON
FROM HANG;
...
-- 3. Cursor hiển thị chi tiết hóa đơn
DECLARE @MAHD VARCHAR(10), @MAHG VARCHAR(10), @TENHG NVARCHAR(50), @SOLUONG INT, @GIABAN MONEY, @THANHTIEN MONEY;

DECLARE DSSOLIEUHOADON CURSOR FOR
SELECT MAHD, MAHG,
       (SELECT TENHG FROM HANG WHERE MAHG = CTHD.MAHG) AS TENHG,
